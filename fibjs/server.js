...
var doc_root = process.argv[3] || process.cwd();
...
var static_server = function (request) {
	if (request.response.status === 404) {
		var p = doc_root + encoding.decodeURI(request.address);
		if (p[-1] !== '//') {
			p += '//';
		}
		if (fs.exists(p + "index.html")) {
			request.response.body = fs.open(p + "index.html");
			request.response.status = 200;
		} else if (fs.exists(p)) {
			var rep = html_str;
			rep += '<script>start("' + request.address + '");</script>';
			if (request.address.lastIndexOf('/') > 0) {
				rep += '<script>addRow("..","..",1,"0 B","");</script>';
			}
			var files = fs.readdir(p);
            ...
            if (files[i].name == "." || files[i].name == "..") {
              continue;
						...
						rep += '<script>addRow("' + files[i].name + '","' + encoding.encodeURIComponent(files[i].name) + '", 1, "0 B", "' + date_str(files[i].mtime) + '");</script>';
					}
				} catch (error) { }
			}
			for (var i = 0; i < files.length; i++) {
				try {
					if (files[i].isFile()) {
						rep += '<script>addRow("' + files[i].name + '","' + encoding.encodeURIComponent(files[i].name) + '", 0, "' + size_str(files[i].size) + '", "' + date_str(files[i].mtime) + '");</script>';	
					}
				} catch (error) { }
			}
			request.response.status = 200;
			request.response.write(rep);
		}
	}
}


if ("-h" == process.argv[2] || "--help" == process.argv[2]) {
  console.log("usage:\n  fibjs server.js [port [docroot]]\n\n\tport\tdefault as 8080\n\tdocroot\tdefault as current working directory");
  process.exit(0);
}

if (!fs.exists(doc_root)) {
  console.log("[" + doc_root + "] is not a valid directory path!");
  process.exit(1);
}


var server = new http.Server(port, new mq.Chain([http.fileHandler(doc_root), static_server]));
console.log("server running at\n\t=> 0.0.0.0:" + port +", doc_root:" + doc_root);
server.run();
